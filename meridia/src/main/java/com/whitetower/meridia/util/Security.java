...
public class Security {
    ...
    Security(@Value("${jwt.expirationMs}") Integer jwtExpirationMs,
        ...
        signingKeyInBytes = Keys.hmacShaKeyFor(signingKey.getBytes(StandardCharsets.UTF_8));
        contentEncryptionKeyInBytes = Keys.hmacShaKeyFor(contentEncryptionKey.getBytes(StandardCharsets.UTF_8)) ;
    ...
    public Boolean validateJwe(String token) {
        try{
            Jwts.parser().verifyWith(signingKeyInBytes).decryptWith(contentEncryptionKeyInBytes).build().parseSignedClaims(token);
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    **/
    ...
    public Boolean validateJweAndSubject(String token, Long id) {
        try{
            Long idInSubject = Long.valueOf(Jwts.parser().verifyWith(signingKeyInBytes).decryptWith(contentEncryptionKeyInBytes).build().parseSignedClaims(token).getPayload().getSubject());
